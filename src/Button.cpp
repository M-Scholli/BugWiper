...
void Button::check_button_state()
{
	...
	if (button_state == HIGH && _button_press == 1 && time_since_last_press >= _debounce_delay ){
		if(time_since_last_press >= _long_delay) _long_pressed=1;
		else _short_pressed=1;
		_button_press=0;
	}
}

byte Button::button_pressed_long()
{
	byte pressed;
...
byte Button::button_pressed_short()
{
	byte pressed;
